...
def test_register_client_creates_folder_structure(tmp_path):
    ...
    # Register client
    client = manager.register_client("CLIA", "Example Client A")

    assert client.code == "CLIA"
    assert client.name == "Example Client A"
    assert client.created_at
    assert client.updated_at

    # Verify folder structure
    client_dir = data_root / "clients" / "CLIA"
...
def test_register_duplicate_client_raises_error(tmp_path):
    ...
    manager.register_client("CLIA", "Example Client A")

    with pytest.raises(ValueError, match="already exists"):
        manager.register_client("CLIA", "Another Example Client A")
...
def test_get_client(tmp_path):
    ...
    client = manager.get_client("CLIA")
    assert client is not None
    assert client.code == "CLIA"
    assert client.name == "Example Client A"

    # Test case insensitive
    client2 = manager.get_client("clia")
    assert client2 is not None
    assert client2.code == "CLIA"
...
def test_list_clients(tmp_path):
    ...
    manager.register_client("CLIA", "Example Client A")
    manager.register_client("CLIB", "Example Client B")
...
def test_get_client_dir(tmp_path):
    ...
    client_dir = manager.get_client_dir("CLIA")
    expected = data_root / "clients" / "CLIA"
    assert client_dir == expected

    # Test normalization
    client_dir2 = manager.get_client_dir("clia")
...
def test_strict_client_isolation(tmp_path):
    ...
    # Get directories
    clia_dir = manager.get_client_dir("CLIA")
    client_b_dir = manager.get_client_dir("CLIB")
    standard_dir = manager.get_standard_dir()

    # Verify directories are completely separate
    assert clia_dir != client_b_dir
    assert clia_dir != standard_dir
    assert client_b_dir != standard_dir

    # Verify no overlap in paths
    assert not str(clia_dir).startswith(str(client_b_dir))
    assert not str(client_b_dir).startswith(str(clia_dir))
    assert not str(clia_dir).startswith(str(standard_dir))
    assert not str(standard_dir).startswith(str(clia_dir))

    # Verify each has its own databases
    clia_assistant_db = clia_dir / "assistant_kb.sqlite"
    clia_kanban_db = clia_dir / "kanban.sqlite"
    client_b_assistant_db = client_b_dir / "assistant_kb.sqlite"
    client_b_kanban_db = client_b_dir / "kanban.sqlite"
    standard_assistant_db = standard_dir / "assistant_kb.sqlite"

    all_dbs = [
        clia_assistant_db,
        clia_kanban_db,
        client_b_assistant_db,
        client_b_kanban_db,
    ...
    with sqlite3.connect(clia_assistant_db) as conn:
        conn.execute("CREATE TABLE test_table (id INTEGER PRIMARY KEY, data TEXT)")
        conn.execute("INSERT INTO test_table (data) VALUES ('CLIA data')")
        conn.commit()

    # Verify other client's DB does not have this table
    with sqlite3.connect(client_b_assistant_db) as conn:
        cursor = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='test_table'"
        )
        assert cursor.fetchone() is None, "CLIB DB must not have CLIA's tables"
