...
def test_import_csv(tmp_path):
    ...
    with open(csv_path, "w", encoding="utf-8-sig", newline="") as f:
        ...
        writer.writerow({
            "ID Tarea": "T-003",
            "Nombre de tarea": "CLIB billing issue",
            "Cliente": "CLIB",
            "Estado": "Analizado",
            "Prioridad": "Baja",
            "Responsable": "Ana",
    ...
    assert result["per_client"]["CLIA"] == 2
    assert result["per_client"]["CLIB"] == 1

    # Verify tickets in CLIA database
    clia_repo = KanbanRepository(data_root / "clients" / "CLIA" / "kanban.sqlite", seed_columns=False)
    clia_tickets = clia_repo.list_tickets()
    assert len(clia_tickets) == 2

    titles = {t.title for t in clia_tickets}
    assert "Fix IDEX timeout" in titles
    assert "Review meter config" in titles

    # Verify status mapping
    idex_ticket = next(t for t in clia_tickets if t.title == "Fix IDEX timeout")
    ...
    closed_ticket = next(t for t in clia_tickets if t.title == "Review meter config")
    assert closed_ticket.status == "CERRADO"

    # Verify CLIB database
    client_b_repo = KanbanRepository(data_root / "clients" / "CLIB" / "kanban.sqlite", seed_columns=False)
    client_b_tickets = client_b_repo.list_tickets()
    assert len(client_b_tickets) == 1
    assert client_b_tickets[0].status == "ANALIZADO"
    assert client_b_tickets[0].priority == "LOW"
