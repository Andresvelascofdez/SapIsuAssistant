...
class TestKanbanFullCRUD:
    ...
    def test_full_ticket_lifecycle(self, kanban_clia):
        ...
        ticket = kanban_clia.update_status(ticket.id, TicketStatus.IN_PROGRESS)
        assert ticket.status == TicketStatus.IN_PROGRESS

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.WAITING)
        assert ticket.status == TicketStatus.WAITING

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.IN_PROGRESS)
        assert ticket.status == TicketStatus.IN_PROGRESS

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.DONE)
    ...
    def test_list_and_filter_tickets(self, kanban_clia):
        ...
        kanban_clia.update_status(t2.id, TicketStatus.IN_PROGRESS)

        all_tickets = kanban_clia.list_tickets()
        open_tickets = kanban_clia.list_tickets(status=TicketStatus.OPEN)
        ip_tickets = kanban_clia.list_tickets(status=TicketStatus.IN_PROGRESS)
