...
- Full chat RAG pipeline: question -> embed -> search -> context -> ancliar
...
@pytest.fixture
def client_clia(client_manager):
    return client_manager.register_client("CLIA", "Stadtwerke Beispiel")


@pytest.fixture
def client_client_b(client_manager):
    return client_manager.register_client("CLIB", "Example Client B Energy")
...
@pytest.fixture
def kb_repo_clia(client_manager, client_clia):
    clia_dir = client_manager.get_client_dir("CLIA")
    return KBItemRepository(clia_dir / "assistant_kb.sqlite")


@pytest.fixture
def kb_repo_client_b(client_manager, client_client_b):
    client_b_dir = client_manager.get_client_dir("CLIB")
    return KBItemRepository(client_b_dir / "assistant_kb.sqlite")
...
@pytest.fixture
def kanban_clia(client_manager, client_clia):
    clia_dir = client_manager.get_client_dir("CLIA")
    return KanbanRepository(clia_dir / "kanban.sqlite")


@pytest.fixture
def kanban_client_b(client_manager, client_client_b):
    client_b_dir = client_manager.get_client_dir("CLIB")
    return KanbanRepository(client_b_dir / "kanban.sqlite")
...
# ===========================================================================
# 2. FULL CHAT RAG PIPELINE: question -> embed -> search -> context -> ancliar
# ===========================================================================

class TestFullChatRAGPipeline:
    """End-to-end chat RAG: embed question -> Qdrant search -> fetch KB -> context -> OpenAI ancliar."""
    ...
    def test_e2e_chat_ancliar(self, mock_qdrant_class, mock_embed_openai, mock_chat_openai,
                             kb_repo_standard):
        """Full RAG pipeline from question to ancliar."""
    ...
    def test_chat_with_multiple_sources(self, mock_qdrant_class, mock_embed_openai,
        ...
        assert "Step 1" in result.ancliar
    ...
    def test_chat_no_results_returns_missing_context(self, mock_qdrant_class, mock_embed_openai,
        ...
        mock_chat_openai.return_value = mock_chat_client
        mock_chat_client.responses.create.return_value = Mock(
            output_text="I don't have enough context to ancliar."
        )
...
class TestOpenAIRequestResponse:
    ...
    def test_chat_response_with_reasoning_effort(self, mock_qdrant_class, mock_embed_openai,
        ...
        mock_chat_client = MagicMock()
        mock_chat_openai.return_value = mock_chat_client
        mock_chat_client.responses.create.return_value = Mock(output_text="Ancliar")
...
class TestKanbanFullCRUD:
    ...
    def test_create_ticket(self, kanban_clia):
        ticket = kanban_clia.create_ticket(
    ...
    def test_full_ticket_lifecycle(self, kanban_clia):
        """EN_PROGRESO -> ANALIZADO -> TESTING -> ANALIZADO -> CERRADO."""
        ticket = kanban_clia.create_ticket(title="IDEX Fix", priority=TicketPriority.HIGH)
        assert ticket.status == TicketStatus.EN_PROGRESO

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.ANALIZADO)
        assert ticket.status == TicketStatus.ANALIZADO

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.TESTING)
        assert ticket.status == TicketStatus.TESTING

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.ANALIZADO)
        assert ticket.status == TicketStatus.ANALIZADO

        ticket = kanban_clia.update_status(ticket.id, TicketStatus.CERRADO)
        assert ticket.status == TicketStatus.CERRADO
        assert ticket.closed_at is not None

        # Verify history
        history = kanban_clia.get_history(ticket.id)
    ...
    def test_update_ticket_fields(self, kanban_clia):
        ticket = kanban_clia.create_ticket(title="Original Title")

        updated = kanban_clia.update_ticket(
            ticket.id,
            title="Updated Title",
    ...
    def test_list_and_filter_tickets(self, kanban_clia):
        kanban_clia.create_ticket(title="T1")
        t2 = kanban_clia.create_ticket(title="T2")
        kanban_clia.update_status(t2.id, TicketStatus.ANALIZADO)

        all_tickets = kanban_clia.list_tickets()
        open_tickets = kanban_clia.list_tickets(status=TicketStatus.EN_PROGRESO)
        ip_tickets = kanban_clia.list_tickets(status=TicketStatus.ANALIZADO)
...
class TestClientIsolation:
    ...
    def test_kb_items_isolated_between_clients(self, kb_repo_clia, kb_repo_client_b):
        """KB items in CLIA are not visible from CLIB."""
        _make_kb_item(kb_repo_clia, title="CLIA Incident", scope="client", client_code="CLIA")
        _make_kb_item(kb_repo_client_b, title="CLIB Incident", scope="client", client_code="CLIB")

        clia_items = kb_repo_clia.list_by_scope("client", client_code="CLIA")
        client_b_items = kb_repo_client_b.list_by_scope("client", client_code="CLIB")

        assert len(clia_items) == 1
        assert clia_items[0].title == "CLIA Incident"
        assert len(client_b_items) == 1
        assert client_b_items[0].title == "CLIB Incident"

    def test_kanban_isolated_between_clients(self, kanban_clia, kanban_client_b):
        """Kanban tickets in CLIA are not visible from CLIB."""
        kanban_clia.create_ticket(title="CLIA Ticket")
        kanban_client_b.create_ticket(title="CLIB Ticket")

        clia_tickets = kanban_clia.list_tickets()
        client_b_tickets = kanban_client_b.list_tickets()

        assert len(clia_tickets) == 1
        assert clia_tickets[0].title == "CLIA Ticket"
        assert len(client_b_tickets) == 1
        assert client_b_tickets[0].title == "CLIB Ticket"

    def test_client_folders_isolated(self, client_manager, client_clia, client_client_b):
        """Each client has its own directory structure."""
        clia_dir = client_manager.get_client_dir("CLIA")
        client_b_dir = client_manager.get_client_dir("CLIB")

        assert clia_dir != client_b_dir
        assert clia_dir.exists()
        assert client_b_dir.exists()
        assert (clia_dir / "assistant_kb.sqlite").exists()
        assert (client_b_dir / "assistant_kb.sqlite").exists()
        assert (clia_dir / "kanban.sqlite").exists()
        assert (client_b_dir / "kanban.sqlite").exists()

    def test_standard_scope_separate_from_clients(self, client_manager, kb_repo_standard, kb_repo_clia):
        """Standard KB items are separate from client items."""
        _make_kb_item(kb_repo_standard, title="Standard Item", scope="standard")
        _make_kb_item(kb_repo_clia, title="Client Item", scope="client", client_code="CLIA")
        ...
        assert len(clia_items) == 1
        assert clia_items[0].title == "Client Item"
    ...
    def test_qdrant_collections_per_client(self, mock_qdrant_class):
        ...
        svc.ensure_collection_exists("client", "CLIA")
        svc.ensure_collection_exists("client", "CLIB")
        ...
        assert "kb_CLIB" in collection_names
...
class TestErrorScenarios:
    ...
    def test_chat_error_on_embedding_failure(self, mock_embed_openai, mock_chat_openai,
        ...
        with pytest.raises(ChatError) as exc_info:
            chat_svc.ancliar("test", kb_repo_standard, "standard", None)
